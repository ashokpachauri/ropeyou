...
<div class="p-3 user_section_bridge">
	...
	?>
			 <div class="col-md-4" id="user_section_bridge_<?php echo $connect_user_id; ?>">
				<b href="profile">
				   <div class="border network-list network-item rounded mb-3">
					  ...
					  <?php
						...
						$m1_query="SELECT * FROM user_joins_user WHERE ((user_id=".$bridge_row['id']." AND r_user_id!=".$_COOKIE['uid'].") OR (r_user_id=".$bridge_row['id']." AND user_id!=".$_COOKIE['uid'].")) AND status=1 AND blocked=0";
						...
						if($num_rows>0)
						{
							$m2_query="SELECT * FROM user_joins_user WHERE ((user_id=".$_COOKIE['uid']." AND r_user_id!=".$bridge_row['id'].") OR (r_user_id=".$_COOKIE['uid']." AND user_id!=".$bridge_row['id'].")) AND status=1 AND blocked=0";
					  ...
					  <div class="network-item-footer py-3 d-flex text-center">
						 <div class="col-6 pl-3 pr-1">
							<button type="button" onclick="DisConnectUser('<?php echo $connect_user_id; ?>');" class="btn btn-primary btn-sm btn-block"> <?php echo $text; ?> </button>
						 </div>
						 <div class="col-6 pr-3 pl-1">
							<button type="button" class="btn btn-outline-primary btn-sm btn-block"> <i class="feather-user-plus"></i> <?php echo $follow; ?> </button>
						 </div>
					  </div>
				   </div>
				</b>
	...
	<?php
		...
		else
		{
			?>
			<div class="col-md-12">
				<h5 style="text-align:center;">There is nothing to show.</h5>
			</div>
			<?php
		}
	?>
